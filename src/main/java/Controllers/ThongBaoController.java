...
public class ThongBaoController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Tạo một cookie
        Cookie myCookie = new Cookie("nhom14", "valueNhom14");
    ...
    private void  listthongbaoadmin(HttpServletRequest request, HttpServletResponse response)
        ...
        myCookie.setMaxAge(3600);  // Thời gian sống của cookie (ví dụ: 1 giờ)
        myCookie.setPath("/");     // Phạm vi của cookie (ví dụ: toàn bộ trang web)
        myCookie.setHttpOnly(true); // Đặt HttpOnly flag, cookie chỉ có thể được
                                    // truy cập thông qua giao thức HTTP và không thể truy cập bằng JavaScript.
        myCookie.setSecure(true);   // Set Secure flag, cookie chỉ được gửi qua kênh an toàn HTTPS
        ...
        if (cookieHeader != null) {
            ...
            cookieHeader = "SameSite=Strict";
        } //cookie chỉ được gửi trong ngữ cảnh của trang web gốc (cùng phương thức, cùng tên miền, cùng cổng)
        response.setHeader("Set-Cookie", cookieHeader);
        List<Thongbao> listthongbao = thongBaoDAO.geAllThongBao();
        request.setAttribute("listthongbao", listthongbao);
